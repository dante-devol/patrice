...
| **Division** | The core axis of *labor* (Writing, Art, Testing, Leadership…); owns an Inherent Role, a default Request Template, and acts as a permission scope. | Department, Category |
...
## Tasks & request templates

| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Task** | The uniform unit of tracked work — a Division (required), optional Team, requester, description, claimant slots, a Request Template, a derived Status, and a Message thread. | Ticket, Job, Item |
...
| **Request Template** | The structured ask a Task carries — an ordered list of typed Questions, seeded from the Division default and locked once any Submission exists. | Form, Survey, Questionnaire |
| **Question** | One Request Template prompt of a fixed type (Detail-Text · Multiline · Text · Numeric · Dropdown · Radio · Attachment) with qualities (range, options, filetypes, required). | Field, Prompt |

## Submission & review lifecycle

| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Submission** | A Claimant's answers to the Request Template, one per Claimant, carrying a per-submission review status. | Response, Entry |
...
| **Request Template Renderer** *(web)* | The component family that walks the seven Question types in **Authoring** mode (build the question set) or **Answer** mode (fill it in). | Question Form, Survey Component |
...
- A **Task** carries one **Request Template** of ordered **Questions**; each **Claimant** produces one **Submission** of **Answers**.
