...
function requestTemplate(questions: QuestionDef[]): RequestTemplateDef {
...
describe('validateSubmission', () => {
  ...
  it('empty request template with no answers → ok (coordination-only division)', async () => {
    const result = await validateSubmission(requestTemplate([]), [], noAttachments);
    expect(result).toEqual({ ok: true });
  });

  it('answer for an unknown question → unknown_question', async () => {
    const result = await validateSubmission(requestTemplate([]), [{ questionId: 'ghost', text: 'hi' }], noAttachments);
    expect(result.ok).toBe(false);
    if (result.ok) return;
